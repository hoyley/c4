...
class Board:
    ...
    def __init__(self, rows=6, cols=7, line_length=4):
        self.rows = rows
        self.cols = cols
        self.line_length = line_length
        self.board = [[Board.EMPTY_TOKEN for c in range(self.cols)] for r in range(self.rows)]
        self.col_counts = [0 for c in range(self.cols)]
    ...
    def _check_win_in_direction(self, player_val, from_location, direction, remaining):
        ...
        return 0 <= new_col < self.cols and 0 <= new_row < self.rows and \
               self.board[new_row][new_col] == player_val and \
               self._check_win_in_direction(player_val, [new_row, new_col], direction, remaining - 1)
