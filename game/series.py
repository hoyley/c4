...
class Series:
    ...
    def __init__(self, num_games: int, player1: Player, player2: Player, game_factory: GameFactory,
                 is_training: bool = False, print_buffer: int = 1000):
        ...
        self._results_ties = 0
        self._results_player1_wins = 0
        self._results_player2_wins = 0
    ...
    def _game_over(self, game):
        if game.winner == self._player1:
            self._results_player1_wins += 1
        elif game.winner == self._player2:
            self._results_player2_wins += 1
        else:
            self._results_ties += 1
        self._games_played += 1

        if self._games_played % self._print_buffer == 0:
            print(self._format_batch())

    def _format_batch(self):
        output = Series._format_result(self._start_time,
                                       self._results_player1_wins - self._last_batch_p1_wins,
                                       self._results_player2_wins - self._last_batch_p2_wins,
                                       self._results_ties - self._last_batch_ties,
                                       self._games_played,
                                       self._num_games,
                                       self._games_played - self._last_batch_games_played,
                                       self._last_batch_start_time)

        self._last_batch_p1_wins = self._results_player1_wins
        self._last_batch_p2_wins = self._results_player2_wins
        self._last_batch_ties = self._results_ties
        self._last_batch_games_played = self._games_played
        self._last_batch_start_time = time.time()
    ...
    def _format_summary(self):
        return Series._format_result(self._start_time,
                                     self._results_player1_wins,
                                     self._results_player2_wins,
                                     self._results_ties,
                                     self._games_played,
                                     self._num_games,
                                     self._num_games,
